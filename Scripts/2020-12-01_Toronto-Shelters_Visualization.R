...
# Data
shelters <- read_rds(here("data", "toronto_shelter.rds"))
...
ggsave(here::here("plots", paste0("toronto shelter-", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")),
       plot = final, device = agg_png(width = 10, height = 11.1, units = "in", res = 300))

ggsave(here("Plots", "toronto-shelter.png"), 
       width = 30, 
       height = 20, 
       units = "in", 
       dev = ragg::agg_png(res = 300))
