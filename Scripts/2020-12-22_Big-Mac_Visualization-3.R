# Pre requisites
library(tidyverse)
library(here) # For smoother data management
library(patchwork) # For combining multiple plots
library(cowplot) # For more 
library(lubridate)
library(scales) 
library(ggtext) # For more element_* options

# Fonts
library(showtext) # For better fonts and font management
font_add_google("MapGlyphs")
showtext_auto()

# Load the data
big_mac <- read_rds(here("data", "bigmac_raw.rds"))

mapping <- read_csv(here("data", "mapglyphs-mapping.csv"))
...
ggdraw(p) +
    ...
    ggsave(here("plots", paste0("big-mac-", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")), dpi = 320, width = 10, height = 10)
