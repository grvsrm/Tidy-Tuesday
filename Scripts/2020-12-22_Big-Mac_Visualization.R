...
eurozone <- tibble(
    iso_a3 = rep("EUZ", length(eurozone_countries)),
...
euro_bigmac <- date_list %>% 
    inner_join(eurozone, by = "iso_a3") %>% 
        left_join(bigmac %>% select(-name), by = c("iso_a3", "date")) %>% 
    mutate(iso_a3 = if_else(!is.na(a3), a3, iso_a3)) %>% 
    select(-a3)


bigmac <- bigmac %>%
    mutate(name = ifelse(name == "Euro area", "Eurozone", name)) %>%
    bind_rows(euro_bigmac)
...
world_map <- map_data("world") %>%
    ...
    left_join(bigmac %>%  filter(date == ymd("2020-07-01")), by = c(a3 = "iso_a3"))
...
chart <- function(country) {
    ...
    min_axis <-
        ifelse(min(data$usd_raw) > 0, 0, min(data$usd_raw)) - 0.15
    max_axis <-
        ifelse(max(data$usd_raw) < 0, 0, max(data$usd_raw)) + 0.15
...
centre <- map_data("world") %>% as_tibble() %>% 
...
ggsave(here::here("plots", paste0("bigmac-", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")),
